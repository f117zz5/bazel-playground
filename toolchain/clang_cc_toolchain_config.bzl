...
def cc_toolchain_config(
    ...
    # line, so take precedence over any user supplied flags through --copts or
    # such.
    ...
    # Apply sysroot for hermetic linux builds.
    if sysroot_path and target_os == "linux":
        compile_flags.append("--sysroot=" + sysroot_path)
        link_flags.append("--sysroot=" + sysroot_path)
        if target_arch == "x86_64":
            link_flags.extend([
                "-Wl,--dynamic-linker={}/lib/ld-linux-x86-64.so.2".format(sysroot_path),
                "-Wl,-rpath,{}/lib".format(sysroot_path),
                "-Wl,-rpath,{}/usr/lib".format(sysroot_path),
                "-Wl,-rpath,{}/lib64".format(sysroot_path),
            ])
